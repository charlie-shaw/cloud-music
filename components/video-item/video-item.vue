...
<view class="video-item" @tap="redirectVideo">
    <view class="cover-box">
		<image class="image" :src="itemData.cover" mode="widthFix" />
        <view class="info"> 
            <view class="count">{{formatCount(itemData.playCount)}}</view>
            <view class="duration">{{formatTime(itemData.mv.videos[0].duration)}}</view>
        </view>
    </view>
    <view class="description">{{itemData.name}}</view>
</view>
...
const redirectVideo=()=>{
	uni.navigateTo({
		url:`/pages/video-detail/video-detail?id=${props.itemData.id}`
	})
}
</script>
